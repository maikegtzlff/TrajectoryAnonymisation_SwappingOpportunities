...
# PREVIOUS SPEED LOOKUP ONLY INCLUDED SHORSTEST PATHS FOR CLOAKING GAPS TO SAVE ON TIME
# I.E., EXISTING SPEED LOOKUP DOES NOT COVER THE FULL ROAD NETWORK

# how many osmids would be covered
#speedlookup_v2 = pd.read_parquet(r'E:\paper2\Data\shortestpath_output\speed_lookup/speedlookup_final.parquet')

#%% how many road segments have speed info in the previous speed lookup?
# find overlaps in osmids
#mask = np.isin(wayids, speedlookup_v2['id'])
#num_covered = mask.sum()
#print("osmids of head tail shortest path in cloaking gaps speed lookup:", num_covered)
# 17363
#all_covered = mask.all()
#print("are all osmids covered?", all_covered) # False
#print("number of osmids with no record in current speedlookup:", len(wayids)-num_covered) # 1427

#%% only keep the osmids in the df that we need now
#print(len(speedlookup_v2)) #78599428
#speedlookup_v3 = speedlookup_v2[speedlookup_v2['id'].isin(wayids)]
#print(len(speedlookup_v3)) #58240461
#speedlookup_v3.head()

#%% actually this one
#speed_stats_u_red = pd.read_parquet(r'E:\paper2\Data\shortestpath_output\speed_lookup_v2/speed_stats_u_red.parquet')
speed_lu = pd.read_parquet(r'E:\paper2\Data\shortestpath_output\speed_lookup_v2/uber_speedlookup.parquet')
speed_lu.head()
#%%
mask = np.isin(wayids, speed_lu['id'])
num_covered = mask.sum()
print("osmids of head tail shortest path in cloaking gaps speed lookup:", num_covered)
# 16678
all_covered = mask.all()
print("are all osmids covered?", all_covered) # False
print("number of osmids with no record in current speedlookup:", len(wayids)-num_covered) # 2112
# not necesarily for the same users though...

#%%
# must add speed data to shortest path df
# 'median_speed_ms'
# 'speed_source'
...
# uid
# need time_bin
#%% clean up shortest path df
sp_t_cswappingl_origsynf_OD_odid_final = sp_t_cswappingl_origsynf_OD_odid_final.drop(columns=['next_u', 'noSegmentMissing', 'orig_x', 'orig_y', 'dest_x' ,'dest_y', 'edge_type', 'oneway'])

sp_t_cswappingl_origsynf_OD_odid_final['orig_point_id_global'] = sp_t_cswappingl_origsynf_OD_odid_final['odid'].str.extract(r'orig_(\d+)_dest')
sp_t_cswappingl_origsynf_OD_odid_final['orig_point_id_global'] = sp_t_cswappingl_origsynf_OD_odid_final['orig_point_id_global'].astype(int)
sp_t_cswappingl_origsynf_OD_odid_final.head()
# len(sp_t_cswappingl_origsynf_OD_odid_final) # 1,283,623
#%% add time_bin of ENTERING mix-zone, i.e. origin point
# did not keep time_bins after swaping
# must add time_bin to origin points

# row_uid equals poimnt_id_unique, not point_id_global which is used for odid
# t_cswappingl_origsynf_OD has both point_id_global and point_id_unique --> add point_id_unique to OD schortest path
# sp_t_cswappingl_origsynf_OD_odid_final orig_point_id_global
point_id_global_dict = dict(zip(t_cswappingl_origsynf_OD['point_id_global'], t_cswappingl_origsynf_OD['point_id_unique']))
sp_t_cswappingl_origsynf_OD_odid_final['point_id_unique'] = sp_t_cswappingl_origsynf_OD_odid_final['orig_point_id_global'].map(point_id_global_dict)
sp_t_cswappingl_origsynf_OD_odid_final.head()

#%% get time bins
t_forSwapping = gpd.read_parquet(r"d:\paper3\FinalCloakedBasedSwapping\t_forSwapping_26723gaps_labelled.parquet")
#%%
point_id_unique_dict = dict(zip(t_cswappingl_origsynf_OD['point_id_unique'], t_cswappingl_origsynf_OD['point_id_global']))
t_forSwapping['point_id_global'] = t_forSwapping['point_id_unique'].map(point_id_unique_dict)
# timestamp (and bin) for each point_id_unique - where needed
# drop the unneded ones
t_forSwapping = t_forSwapping.dropna(subset=['point_id_global'])
t_forSwapping['point_id_global'] = t_forSwapping['point_id_global'].astype(int) 
t_forSwapping[['point_id_unique', 'point_id_global','datetime_loc_tz', 'time_bin','time_bin_label']].head()

#%% time_bin is based on unix_timestamnp which is the same to datetime_loc_tz, i.e. the raw timestamp not after adding sym points to clk gaps
sp_t_cswappingl_origsynf_OD_odid_final = sp_t_cswappingl_origsynf_OD_odid_final.merge(t_forSwapping[['point_id_unique', 'datetime_loc_tz', 'time_bin','time_bin_label']].copy(), on ='point_id_unique', how='left')
sp_t_cswappingl_origsynf_OD_odid_final.head()

#%%
sp_t_cswappingl_origsynf_OD_odid_final.to_parquet(r'E:\paper3\FinalCloakedBasedSwapping\shortestPath_CloakedBasedSwapping/headtailOD_shortestPath_origTimebins.parquet')

#%% add available speed data
# cannot be uid specific anymore as uid has changed
# no seasonal information

# speed_lu time bins are different to mine!
print(speed_lu.hour_of_day.unique())
print(speed_lu.speed_source.unique())
#%% look at "all users, no season, time of day"
speed_lu[speed_lu['speed_source'] == "all users, no season, time of day"] # 19853766 
# I don't think this is the speed lookup I should be using

#%% look at other speed lookups
speed_stats_u_red = pd.read_parquet(r'E:\paper2\Data\shortestpath_output\speed_lookup_v2/speed_stats_u_red.parquet')
speed_stats_u_red.head()
#%%
speed_stats_u_red.season.unique() # not "all" - array(['autumn', 'spring', 'summer', 'winter'], dtype=object)
#%%
speedlookup_v3.season.unique() # array(['autumn', 'spring', 'summer', 'winter', None], dtype=object)
#%% must look at previous versions of speed lookup
#maxspeed_speed_stats_seasons_h_u.to_parquet(r'E:\paper2\Data\shortestpath_output\speed_lookup/maxspeed_speed_stats_seasons_h_u_source.parquet')
#maxspeed_speed_stats_seasons_h_u.to_parquet(r'E:\paper2\Data\shortestpath_output\speed_lookup/maxspeed_speed_stats_seasons_h_u_ram_reduced.parquet')
maxspeed_speed_stats_seasons_h_u = pd.read_parquet(r'E:\paper2\Data\shortestpath_output\speed_lookup/maxspeed_speed_stats_seasons_h_u_ForMerge.parquet')
maxspeed_speed_stats_seasons_h_u.head()
#%%
#%%  fill the remainder with maxspeed info from OSM
# but which column?
edges = gpd.read_parquet(r"D:\paper2\Data\Output\cloaking_2sig_100150\traj_cloaked_anotated_mapmatched\OriginDestination_CloakingAreas\NetworkShortestPath/edges.parquet")
edges = edges[['id', 'maxspeed']].copy()

# must also take time bins into account
# do not have seasons anymore --> no seasno column
